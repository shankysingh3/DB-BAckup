...
POPPLER_PATH = os.getenv('POPPLER_PATH', None)  # e.g., C:\Program Files\poppler\poppler-25.07.0\Library\bin
...
def mark_processing_failed(conn, queue_id: str, document_id: str,
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Use valid extraction_status values: not_started, in_progress, completed, error
            doc_status = 'in_progress' if should_retry else 'error'
            ...
            # Update document (use valid extraction_status)
            cur.execute("""
                UPDATE document_uploads
                SET
                    extraction_status = %s,
...
def match_emission_factor_scope1(conn, fuel_type: str, unit: str = None) -> Optional[str]:
    """
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # Try exact match with unit first (if unit provided)
            if unit:
            ...
            # Fallback: Try match without unit
            cur.execute("""
                SELECT ef.id
                FROM emission_factors ef
                JOIN emission_factor_classifications efc ON ef.id = efc.emission_factor_id
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            if energy_type.lower() in ['electricity', 'electric']:
                ...
                site = cur.fetchone()
                if not site or not site['grid_region']:
                    logger.warning(f"Site {site_id} has no grid region")
                    return None

                # Match grid region for electricity (uses grid_regions table)
                cur.execute("""
                    SELECT id
                    FROM grid_regions
                    WHERE code = %s
                      AND CURRENT_DATE BETWEEN validity_start_date AND validity_end_date
                    LIMIT 1
                """, (site['grid_region'],))
...
def insert_scope1_emission(conn, document: Dict, extracted_data: Dict,
    ...
    """
    try:
        # Extract year and month from date (use current date if not found)
        date_field = extracted_data.get('invoice_date') or extracted_data.get('billing_period_start')
        if not date_field:
            # Use current date as fallback
            logger.warning("No date field found in extracted data, using current date")
...
def insert_scope2_emission(conn, document: Dict, extracted_data: Dict,
    ...
    """
    try:
        # Extract year and month from billing period (use current date if not found)
        start_date = extracted_data.get('billing_period_start')
        if not start_date:
            # Use current date as fallback
            logger.warning("No billing_period_start found in extracted data, using current date")
            date_obj = datetime.now()
        else:
...
def process_document(document: Dict) -> bool:
    ...
    try:
        ...
        if document['scope'] == 1:
            ...
            unit = extracted_data.get('consumption_unit') or extracted_data.get('unit')
            if fuel_type:
                emission_factor_id = match_emission_factor_scope1(conn, fuel_type, unit)
